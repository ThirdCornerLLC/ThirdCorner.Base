...
// ReSharper restore RedundantEmptyFinallyBlock
        ...
        private Type CreateDynamicClass(IList<DynamicProperty> properties)
        {
            ...
            try
            {
                string typeName = string.Format("DynamicClass{0}", (_classCount + 1));
#if ENABLE_LINQ_PARTIAL_TRUST
                new ReflectionPermission(PermissionState.Unrestricted).Assert();
#endif
                try
...
// ReSharper restore RedundantEmptyFinallyBlock
        ...
        private static FieldInfo[] GenerateProperties(TypeBuilder tb, IList<DynamicProperty> properties)
        {
            ...
            {
                var dp = properties[i];
                var fb = tb.DefineField(string.Format("_{0}", dp.Name), dp.Type, FieldAttributes.Private);
                var pb = tb.DefineProperty(dp.Name, PropertyAttributes.HasDefault, dp.Type, null);
                var mbGet = tb.DefineMethod(string.Format("get_{0}", dp.Name),
                ...
                var mbSet = tb.DefineMethod(string.Format("set_{0}", dp.Name),
                                                      MethodAttributes.Public | MethodAttributes.SpecialName |
                                                      MethodAttributes.HideBySig,
                                                      null, new[] {dp.Type});
...
// ReSharper restore UnusedMember.Local
        ...
        public ExpressionParser(IList<ParameterExpression> parameters, string expression, object[] values)
        {
...
#pragma warning restore 0219
        ...
        private Expression ParseUnary()
        {
                ...
                TokenIdentifierIs("not"))
            {
                ...
                {
                    _token.Text = string.Format("-{0}", _token.Text);
                    _token.Pos = op.Pos;
                    return ParsePrimary();
                }
                var expr = ParseUnary();
        ...
        private Expression ParseIntegerLiteral()
        {
            ...
            if (text[0] != '-')
            {
                ...
                return value <= Int64.MaxValue ? CreateLiteral((long) value, text) : CreateLiteral(value, text);
            }
            else
            {
                long value;
        ...
        private static Type FindGenericType(Type generic, Type type)
        {
            while (type != null && type != typeof (object))
            {
                ...
                if (generic.IsInterface)
                {
                    foreach (
                        var found in
                            type.GetInterfaces().Select(intfType => FindGenericType(generic, intfType)).Where(
                                found => found != null))
                        return found;
                }
                type = type.BaseType;
        ...
        private Expression ParseAggregate(Expression instance, Type elementType, string methodName, int errorPos)
        {
            ...
            var typeArgs = signature.Name == "Min" || signature.Name == "Max"
                                  ? new[] {elementType, args[0].Type}
                                  : new[] {elementType};
        ...
        private static MemberInfo FindPropertyOrField(Type type, string memberName, bool staticAccess)
        {
            ...
            return (SelfAndBaseTypes(type).Select(
                t =>
                t.FindMembers(MemberTypes.Property | MemberTypes.Field, flags, Type.FilterNameIgnoreCase, memberName)).
                Where(members => members.Length != 0).Select(members => members[0])).FirstOrDefault();
        }
        ...
        private int FindMethod(Type type, string methodName, bool staticAccess, Expression[] args, out MethodBase method)
        {
            var flags = BindingFlags.Public | BindingFlags.DeclaredOnly |
                                 (staticAccess ? BindingFlags.Static : BindingFlags.Instance);
            foreach (var members in SelfAndBaseTypes(type).Select(t => t.FindMembers(MemberTypes.Method,
        ...
        {
            foreach (var methods in
                SelfAndBaseTypes(type).Select(t => t.GetDefaultMembers()).Where(members => members.Length != 0).Select(
                    members => members.
                                   OfType<PropertyInfo>().
